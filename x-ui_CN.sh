...
install() {
    bash <(curl -Ls https://raw.githubusercontent.com/tyrantcwj/x-ui/main/install_CN.sh)
    if [[ $? == 0 ]]; then
        if [[ $# == 0 ]]; then
            start
        else
...
update_shell() {
    wget -O /usr/bin/x-ui -N --no-check-certificate https://github.com/tyrantcwj/x-ui/raw/main/x-ui_CN.sh
    if [[ $? != 0 ]]; then
        echo ""
